...
axiomatization
  elem_of :: "[elem, set] \<Rightarrow> prop" (infix "\<in>" 50) and
  rel_of  :: "[rel, set, set] \<Rightarrow> prop" ("(_ : _ \<succ> _)" [51, 0, 51] 50) and
  holds   :: "[rel, elem, elem] \<Rightarrow> prop" ("(_'(_, _'))" [100, 0, 0]) where
...
abbreviation "not_elem_of" :: "[elem, set] \<Rightarrow> prop" (infix "\<notin>" 50)
...
no_notation
  All (binder "\<forall>" 0) and
  Ex  (binder "\<exists>" 0) and
  Ex1 (binder "\<exists>!" 0)
notation
  All (binder "\<forall>\<forall>" 0) and
  Ex  (binder "\<exists>\<exists>" 0) and
  Ex1 (binder "\<exists>\<exists>!" 0)

abbreviation all_set :: "(set \<Rightarrow> prop) \<Rightarrow> prop" (binder "\<forall>" 0)
  where "(\<forall>x. PROP P x) \<equiv> (\<forall>\<forall>x::set. PROP P x)"

abbreviation ex_set :: "(set \<Rightarrow> prop) \<Rightarrow> prop" (binder "\<exists>" 0)
  where "(\<exists>x. PROP P x) \<equiv> (\<exists>\<exists>x::set. PROP P x)"

abbreviation ex1_set :: "(set \<Rightarrow> prop) \<Rightarrow> prop" (binder "\<exists>!" 0)
  where "(\<exists>!x. PROP P x) \<equiv> (\<exists>\<exists>!x::set. PROP P x)"

abbreviation all_elem :: "[set, elem \<Rightarrow> prop] \<Rightarrow> prop"
  where "all_elem X P \<equiv> (\<forall>\<forall>x. x \<in> X \<Longrightarrow> PROP P x)"

abbreviation ex_elem :: "[set, elem \<Rightarrow> prop] \<Rightarrow> prop"
  where "ex_elem X P \<equiv> (\<exists>\<exists>x. x \<in> X \<and> PROP P x)"

abbreviation ex1_elem :: "[set, elem \<Rightarrow> prop] \<Rightarrow> prop"
  where "ex1_elem X P \<equiv> (\<exists>\<exists>!x. x \<in> X \<and> PROP P x)"

abbreviation all_rel :: "[set, set, rel \<Rightarrow> prop] \<Rightarrow> prop"
  where "all_rel A B P \<equiv> (\<forall>\<forall>\<phi>. \<phi>: A \<succ> B \<Longrightarrow> PROP P \<phi>)"

abbreviation ex_rel :: "[set, set, rel \<Rightarrow> prop] \<Rightarrow> prop"
  where "ex_rel A B P \<equiv> (\<exists>\<exists>\<phi>. \<phi>: A \<succ> B \<and> PROP P \<phi>)"

abbreviation ex1_rel :: "[set, set, rel \<Rightarrow> prop] \<Rightarrow> prop"
  where "ex1_rel A B P \<equiv> (\<exists>\<exists>!\<phi>. \<phi>: A \<succ> B \<and> PROP P \<phi>)"


syntax
  "_all_elem"    :: "[pttrn, set, prop] \<Rightarrow> prop"      ("(\<forall>_ \<in> _./ _)" [0, 0, 0] 1)
  "_ex_elem"     :: "[pttrn, set, prop] \<Rightarrow> prop"      ("(\<exists>_ \<in> _./ _)" [0, 0, 0] 1)
  "_ex1_elem"    :: "[pttrn, set, prop] \<Rightarrow> prop"      ("(\<exists>!_ \<in> _./ _)" [0, 0, 0] 1)
  "_all_rel"     :: "[pttrn, set, set, prop] \<Rightarrow> prop" ("(\<forall>_: _ \<succ> _./ _)" [0, 0, 0, 0] 1)
  "_ex_rel"      :: "[pttrn, set, set, prop] \<Rightarrow> prop" ("(\<exists>_: _ \<succ> _./ _)" [0, 0, 0, 0] 1)
  "_ex1_rel"     :: "[pttrn, set, set, prop] \<Rightarrow> prop" ("(\<exists>!_: _ \<succ> _./ _)" [0, 0, 0, 0] 1)

  "_all_elem_at" :: "[pttrn, set, aprop] \<Rightarrow> prop"      ("(\<forall>_ \<in> _./ _)" [0, 0, 0] 1)
  "_ex_elem_at"  :: "[pttrn, set, aprop] \<Rightarrow> prop"      ("(\<exists>_ \<in> _./ _)" [0, 0, 0] 1)
  "_ex1_elem_at" :: "[pttrn, set, aprop] \<Rightarrow> prop"      ("(\<exists>!_ \<in> _./ _)" [0, 0, 0] 1)
  "_all_rel_at"  :: "[pttrn, set, set, aprop] \<Rightarrow> prop" ("(\<forall>_: _ \<succ> _./ _)" [0, 0, 0, 0] 1)
  "_ex_rel_at"   :: "[pttrn, set, set, aprop] \<Rightarrow> prop" ("(\<exists>_: _ \<succ> _./ _)" [0, 0, 0, 0] 1)
  "_ex1_rel_at"  :: "[pttrn, set, set, aprop] \<Rightarrow> prop" ("(\<exists>!_: _ \<succ> _./ _)" [0, 0, 0, 0] 1)

translations
  "_all_elem x X P"     \<rightleftharpoons> "CONST all_elem X (\<lambda>x. P)"
  "_all_elem_at x X P"  \<rightleftharpoons> "CONST all_elem X (\<lambda>x. P)"

  "_ex_elem x X P"      \<rightleftharpoons> "CONST ex_elem X (\<lambda>x. P)"
  "_ex_elem_at x X P"   \<rightleftharpoons> "CONST ex_elem X (\<lambda>x. P)"

  "_ex1_elem x X P"     \<rightleftharpoons> "CONST ex1_elem X (\<lambda>x. P)"
  "_ex1_elem_at x X P"  \<rightleftharpoons> "CONST ex1_elem X (\<lambda>x. P)"

  "_all_rel \<phi> A B P"    \<rightleftharpoons> "CONST all_rel A B (\<lambda>\<phi>. P)"
  "_all_rel_at \<phi> A B P" \<rightleftharpoons> "CONST all_rel A B (\<lambda>\<phi>. P)"

  "_ex_rel \<phi> A B P"     \<rightleftharpoons> "CONST ex_rel A B (\<lambda>\<phi>. P)"
  "_ex_rel_at \<phi> A B P"  \<rightleftharpoons> "CONST ex_rel A B (\<lambda>\<phi>. P)"

  "_ex1_rel \<phi> A B P"    \<rightleftharpoons> "CONST ex1_rel A B (\<lambda>\<phi>. P)"
  "_ex1_rel_at \<phi> A B P" \<rightleftharpoons> "CONST ex1_rel A B (\<lambda>\<phi>. P)"


subsubsection \<open>Definite description\<close>

axiomatization
  the_set  :: "(set \<Rightarrow> prop) \<Rightarrow> set" and
  the_elem :: "[set, elem \<Rightarrow> prop] \<Rightarrow> elem" and
  the_rel  :: "[set, set, rel \<Rightarrow> prop] \<Rightarrow> rel" where

  the_set_def: "\<exists>!X. PROP P X \<Longrightarrow> PROP P (the_set P)" and

  the_elem_elem_of: "\<exists>!x \<in> X. PROP Q x \<Longrightarrow> the_elem X Q \<in> X" and
  the_elem_sat: "\<exists>!x \<in> X. PROP Q x \<Longrightarrow> PROP Q (the_elem X Q)" and

  the_rel_sort: "\<exists>!\<phi>: A \<succ> B. PROP R \<phi> \<Longrightarrow> the_rel A B R: A \<succ> B" and
  the_rel_sat: "\<exists>!\<phi>: A \<succ> B. PROP R \<phi> \<Longrightarrow> PROP R (the_rel A B R)"
...
definition rel_inj :: "[rel, set, set] \<Rightarrow> prop" ("(_ : _ \<succ> _ injective)")
  where "\<phi>: A \<succ> B injective \<equiv> (\<forall>a \<in> A. \<forall>a' \<in> A. (\<exists>b \<in> B. \<phi>(a, b) \<and> \<phi>(a', b)) \<Longrightarrow> a = a')"

definition rel_surj :: "[rel, set, set] \<Rightarrow> prop" ("(_ : _ \<succ> _ surjective)")
  where "\<phi>: A \<succ> B surjective \<equiv> (\<forall>b \<in> B. \<exists>a \<in> A. \<phi>(a, b))"


subsubsection \<open>Functions\<close>

abbreviation is_fun :: "[rel, set, set] \<Rightarrow> prop" ("(_ : _ \<rightarrow> _)"[51, 0, 51] 50)
  where "f: A \<rightarrow> B \<equiv> f: A \<succ> B \<and> (\<forall>x \<in> A. \<exists>!y \<in> B. f(x, y))"

abbreviation all_fun :: "[set, set, rel \<Rightarrow> prop] \<Rightarrow> prop"
  where "all_fun A B P \<equiv> (\<forall>\<forall>f. f: A \<rightarrow> B \<Longrightarrow> PROP P f)"

abbreviation ex_fun :: "[set, set, rel \<Rightarrow> prop] \<Rightarrow> prop"
  where "ex_fun A B P \<equiv> (\<exists>\<exists>f. f: A \<rightarrow> B \<and> PROP P f)"

abbreviation ex1_fun :: "[set, set, rel \<Rightarrow> prop] \<Rightarrow> prop"
...
abbreviation (input) is_tabulation_of :: "[set, rel, rel, rel, set, set] \<Rightarrow> prop"
...
definition subset_of :: "[set, set] \<Rightarrow> prop" (infix "\<subseteq>" 50)
